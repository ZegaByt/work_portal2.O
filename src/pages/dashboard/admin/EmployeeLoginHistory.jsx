...
function EmployeeLoginHistory() {
  ...
  // Fetch login history and filter by assigned employees
  useEffect(() => {
    const fetchAllLoginHistory = async () => {
      ...
      try {
        ...
        // Fetch login history for all employees
        ...
        // Filter login history by assigned employee IDs
        const employeeIds = employees.map((emp) => emp.user_id);
        const filteredHistory = response.data.results.filter((entry) =>
          employeeIds.includes(entry.user_details?.user_id)
        );

        setAllLoginHistory(filteredHistory);
        toast.success('Login history loaded successfully!', { duration: 3000 });
      } catch (err) {
        console.error('Error fetching employee login history:', {
          message: err.message,
          status: err.response?.status,
          data: err.response?.data,
        });
        let errorMessage = err.message || 'Failed to load employee login history.';
        if (err.response?.status === 404) {
          errorMessage = 'Login history endpoint not found. Please verify the API configuration.';
  ...
  useEffect(() => {
    ...
    const searchedData = dataToFilter
      .filter((entry) => {
  ...
  return (
    <main className="p-2 sm:p-0 bg-gray-50 min-h-screen">
