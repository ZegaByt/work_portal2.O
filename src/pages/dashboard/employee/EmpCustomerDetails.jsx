...
// Import ECustomerDetails, FollowUpNotes, EmpCustomerInterests lazily
const LazyECustomerDetails = lazy(() => import('./ECustomerDetails'));
const LazyFollowUpNotes = lazy(() => import('./FollowUpNotes'));
const LazyEmpCustomerInterests = lazy(() => import('./EmpCustomerInterests'));

import { Badge } from '@/components/ui/badge';

// SkeletonLoader Component for fallbacks
...
// ProgressCircle component (unchanged)
...
// Dropdown endpoints (unchanged, but moved inside component for useMemo if needed)
const endpoints = [
...
// Field configurations (unchanged, but will be passed to child components)
const fieldConfig = {
  personal: {
    fields: {
  ...
  career: {
    fields: {
      ...
      job_country: { type: 'select', endpoint: 'country' }, // Changed to 'country'
      job_state: { type: 'select', endpoint: 'state', dependsOn: 'job_country' }, // Added dependsOn
      job_city: { type: 'select', endpoint: 'city', dependsOn: 'job_state' }, // Added dependsOn
    },
  },
  astro: {
    fields: {
  ...
  religion: {
    fields: {
      ...
      visa_country: { type: 'select', endpoint: 'country' }, // Changed to 'country'
  ...
  location: {
    fields: {
      ...
      state: { type: 'select', endpoint: 'state', dependsOn: 'country' }, // Added dependsOn
      district: { type: 'select', endpoint: 'district', dependsOn: 'state' }, // Added dependsOn
      city: { type: 'select', endpoint: 'city', dependsOn: 'district' }, // Added dependsOn
      mandal: { type: 'text' },
      ...
      pincode: { type: 'text' },
      google_map_location: { type: 'url', render: 'url_button' }, // Changed render to url_button
      family_google_map: { type: 'url', render: 'url_button' }, // Changed render to url_button
      native_country: { type: 'select', endpoint: 'country' },
      native_state: { type: 'select', endpoint: 'state', dependsOn: 'native_country' }, // Added dependsOn
      native_district: { type: 'select', endpoint: 'district', dependsOn: 'native_state' }, // Added dependsOn
      native_city: { type: 'select', endpoint: 'city', dependsOn: 'native_district' }, // Added dependsOn
  ...
  preferences: {
    fields: {
      ...
      pref_states: { type: 'multiselect', endpoint: 'state', dependsOn: 'pref_countries' },
      pref_districts: { type: 'multiselect', endpoint: 'district', dependsOn: 'pref_states' },
      pref_cities: { type: 'multiselect', endpoint: 'city', dependsOn: 'pref_districts' },
      pref_job_countries: { type: 'multiselect', endpoint: 'country' },
      pref_job_states: { type: 'multiselect', endpoint: 'state', dependsOn: 'pref_job_countries' },
      pref_job_cities: { type: 'multiselect', endpoint: 'city', dependsOn: 'pref_job_states' },
      pref_citizenship_countries: { type: 'multiselect', endpoint: 'citizenship' },
      pref_visa_countries: { type: 'multiselect', endpoint: 'country' }, // Changed to 'country'
...
const EmpCustomerDetails = React.memo(() => { // Wrapped with React.memo
  ...
  // Update setCurrentCustomerHeaderInfo when activeTab changes
  useEffect(() => {
    if (customer) { // Only update if customer data is loaded
      setCurrentCustomerHeaderInfo(prev => ({
        ...prev,
        currentActiveTab: activeTab,
      }));
    }
  }, [activeTab, customer, setCurrentCustomerHeaderInfo]); // Dependencies
  ...
  // Get option name for display (unchanged)
  ...
  const fetchCustomerDetails = useCallback(async () => {
    ...
    try {
      ...
      // Calculate completion percentage based on the structure of fieldConfig
      let totalFields = 0;
      let filledFields = 0;

      Object.values(fieldConfig).forEach(section => {
        Object.entries(section.fields).forEach(([fieldName, config]) => {
          totalFields++; // Count every field in the config
      ...
      const headerInfo = {
        full_name: customerData.full_name || `${customerData.first_name || ''} ${customer.surname || ''}`.trim(),
        ...
        currentActiveTab: activeTab, // This will be updated in the next render cycle, but it's okay for initial
      };
  ...
  }, [user_id, navigate, setCurrentCustomerHeaderInfo]); // Removed activeTab from dependencies
  ...
  const renderContent = useCallback(() => { // Memoized renderContent
    ...
    switch (activeTab) {
      case 'CustomerDetails':
        return <LazyECustomerDetails user_id={user_id} />;
      case 'Followups':
        return <LazyFollowUpNotes user_id={user_id} />;
      case 'Interests':
        return <LazyEmpCustomerInterests user_id={user_id} />;
      default:
        return null;
    }
  }, [activeTab, loading, loadingDropdowns, user_id]); // Dependencies for renderContent
  ...
  const getBasicProfileData = useCallback(() => {
  ...
  const getFullProfileData = useCallback(() => {
    if (!customer) return { message: 'No customer data available' };
    ...
    const excludeFields = [
      ...
      'assigned_employee_name',"is_staff",
      "is_active",
      "created_at",
      "updated_at"
    ];
  ...
  const handleCopy = useCallback((data) => {
    ...
    // Format data as plain text in a form-like structure
    const formatDataAsText = (data) => {
      const lines = [];
      // Currency fields to format
      const currencyFields = ['total_property_value', 'payment_amount', 'settlement_amount']; // Added payment and settlement amounts
      ...
      if (modalContent === 'basic') {
        ...
        Object.entries(data).forEach(([key, value]) => {
          if (!excludeFields.includes(key) && value && value !== 'Not provided') {
            ...
            if (fieldToEndpointMap[key] && dropdownData[fieldToEndpointMap[key]]) {
              formattedValue = getOptionName(fieldToEndpointMap[key], value);
            }
                ...
                if (Array.isArray(value)) {
                  if (fieldToEndpointMap[key] && dropdownData[fieldToEndpointMap[key]]) {
                    formattedValue = value.length > 0
                      ? value.map(id => getOptionName(fieldToEndpointMap[key], id)).join(', ')
                      : 'None';
                  } else {
  ...
  }, [modalContent, dropdownData, getOptionName]);
    ...
    return (
      <div className="space-y-6">
        {modalContent === 'basic' ? (
          ...
          <pre className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg overflow-auto text-sm text-gray-800 dark:text-gray-200 max-h-80">
  ...
  }, [modalContent, getBasicProfileData, getFullProfileData, handleCopy, copied, customer, getOptionName]);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-6 mb-6">
          {loading || loadingDropdowns ? (
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
              ...
              <SkeletonLoader className="w-24 h-24 sm:w-32 sm:h-32 rounded-full" />
            </div>
          ) : (
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
              {customer.profile_photos ? (
